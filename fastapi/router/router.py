...
@main_router.post("/translate_kor", response_model=ResponseBody)
def translate_kor(content: str = Body(..., media_type="text/plain")):
    celery_result = celery_translate_kor.delay(content)
    print(celery_result)
...
def translate_eng(content: str = Body(..., media_type="text/plain")):
    celery_result = celery_translate_eng.delay(content)
    print(celery_result)
    return ResponseBody(msg = celery_result.state, body=celery_result.id)
